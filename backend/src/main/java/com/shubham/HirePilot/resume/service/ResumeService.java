...
public class ResumeService {
    ...
    public List<Resume> uploadMultipleResumes(List<MultipartFile> files, User uploadedBy) {

        List<Resume> uploadedResumes = new java.util.ArrayList<>();

        for (MultipartFile file : files) {
            try {
                Resume resume = uploadResume(file, uploadedBy);   // wahi purana single-upload logic reuse
                uploadedResumes.add(resume);
            } catch (Exception e) {
                // Ek file fail ho bhi jaye (corrupt PDF, wrong type), baaki resumes rukne nahi chahiye
                log.error("Failed to upload resume {}: {}", file.getOriginalFilename(), e.getMessage());
            }
        }

        log.info("Batch upload complete: {}/{} resumes uploaded successfully",
                uploadedResumes.size(), files.size());
